...
from torchvision.models import resnet18, ResNet18_Weights, resnet34, ResNet34_Weights, resnet50
...
class ChannelProjector(nn.Module): # currently not in use
    """
...
class ResNetFeatureExtractor(nn.Module): # not in use
...
class HSResNet34(nn.Module): 
    """
    ResNet34 for hyperspectral data with ViT-style FC head
    Uses standard ResNet34 backbone (no pretrained weights) for fair comparison with ViT
    """
    def __init__(self, input_channels=141, num_outputs=6, output_channels=141):
        super(HSResNet34, self).__init__()
        
        # Load standard ResNet34 w/out pretrained weights
        # Use torchvision's resnet34 model
        self.resnet = models.resnet34(pretrained=False)
        
        # Modify first conv layer to accept hyperspectral input (141 channels)
        # This matches the input channels of the hyperspectral data - Claude-assisted Snippet
        self.resnet.conv1 = nn.Conv2d(
            input_channels,  # 141 hyperspectral channels to match input
            64,             
            kernel_size=7, 
            stride=2, 
            padding=3, 
        ...
        # Project ResNet's 512 features to 1024 to match FCLayer 
        # This is to align with the ViT architecture's feature dimension - Claude-assisted Snippet
        self.feature_projector = nn.Linear(512, 1024)
        
        # Add the same FC block structure as ViT (with identical dimensions)
    ...
    def forward(self, x):
        # Pass through ResNet backbone (standard ResNet34 forward pass)
        x = self.resnet(x)
        
        # Project to 1024 features to match ViT
        x = self.feature_projector(x)
        
        # Apply FC blocks with residual connections (exactly like ViT)
        # Claude-assisted Snippet: Residual connections are added to each block
        identity = x
...
class HSResNet50(nn.Module): # Modified using Claude assistance
    """
    ResNet50 for hyperspectral data with ViT-style FC head
    Uses standard ResNet50 backbone (no pretrained weights) for fair comparison with ViT
    """
    def __init__(self, input_channels=141, num_outputs=6, output_channels=141):
        super(HSResNet50, self).__init__()
        
        # Load standard ResNet50 w/out pretrained weights
